...
@property (nonatomic, retain) id viewWillTransitionToSize;
...
@dynamic viewWillTransitionToSize;
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator
{
    ...
    [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext> context) {
        [[NSNotificationCenter defaultCenter]
                postNotificationName:@"onOrientationChange"
                object:self];
    } completion:nil];
}
...
- (void)pluginInitialize
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(statusBarDidChangeFrame:) name: @"onOrientationChange" object:nil];
...
- (UIInterfaceOrientation)getSatatusBarOrientation
{
    if (@available(iOS 13.0, *)) {
        UIWindow *firstWindow = [[[UIApplication sharedApplication] windows] firstObject];
        if (firstWindow == nil) { return NO; }
...
- (void) initializeStatusBarBackgroundView
{
    ...
    if ([self getSatatusBarOrientation] == UIInterfaceOrientationPortraitUpsideDown &&
...
- (void) dealloc
{
    ...
    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"onOrientationChange" object:nil];
}
